...
def action_proposal(path: str) -> dict:
    """Execute a JSON proposal generated by the background review process."""
    p = Path(path)
    if not p.exists():
        return {"success": False, "error": f"Proposal file not found: {path}"}
    try:
        proposal = json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        return {"success": False, "error": f"Failed to parse proposal JSON: {e}"}

    action = proposal.get("action")
    name = proposal.get("name", "")
    content = proposal.get("content", "")
    local = proposal.get("local", False)

    if action == "create":
        return action_create(name, content, category=proposal.get("category"), local=local)
    elif action == "edit":
        return action_edit(name, content, local=local)
    else:
        return {"success": False, "error": f"Unsupported proposal action: {action}"}
...
def main() -> int:
    ...
    proposal_p = subparsers.add_parser("proposal")
    proposal_p.add_argument("--path", required=True, help="Path to a JSON proposal file")
    ...
    if action == "create":
        ...
        result = action_proposal(args.path)
    else:
